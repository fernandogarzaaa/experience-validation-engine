...
describe("attention model", () => {
  ...
  it("salience rewards large, high-contrast, warning-colored elements", () => {
    const big = el(0, { text: "Delete", box: { x: 400, y: 20, width: 300, height: 80 }, color: "#ffffff", backgroundColor: "#d92020" });
    const small = el(1, { text: "footer note", box: { x: 0, y: 780, width: 60, height: 12 }, color: "#cccccc", backgroundColor: "#ffffff" });
  ...
  it("attends only a subset of a dense screen (selective attention)", () => {
    const elements = Array.from({ length: 40 }, (_, i) =>
      el(i, { text: `item ${i}`, interactive: true, role: "link", box: { x: 20, y: 20 + i * 18, width: 120, height: 16 } }),
    );
  ...
  it("attendedPercept restricts elements to those attended", () => {
    const elements = Array.from({ length: 30 }, (_, i) =>
      el(i, { text: `x${i}`, interactive: true, box: { x: 20, y: 20 + i * 20, width: 100, height: 16 } }),
    );
  ...
  it("detects unattended changes as missed changes (change blindness)", () => {
    const before = percept(
      Array.from({ length: 25 }, (_, i) => el(i, { text: `row ${i}`, box: { x: 20, y: 20 + i * 26, width: 200, height: 20 } })),
    );
    // Change a low-salience element far down; a focused reader may miss it.
    const afterEls = before.elements.map((e) => (e.id === 22 ? { ...e, text: "row 22 CHANGED" } : e));
  ...
  it("strong goal focus narrows attention (inattentional blindness)", () => {
    const goalEl = el(0, { text: "Checkout now", interactive: true, role: "button", box: { x: 400, y: 30, width: 200, height: 50 } });
    const distractors = Array.from({ length: 20 }, (_, i) =>
      el(i + 1, { text: `promo ${i}`, interactive: true, box: { x: 20, y: 100 + i * 20, width: 120, height: 16 } }),
    );
