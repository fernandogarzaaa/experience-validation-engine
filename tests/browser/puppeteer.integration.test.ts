 ...
 *
 * Mirrors `tests/browser/playwright.integration.test.ts` exactly — same
 * fixture site, same assertions — because `PuppeteerAdapter` is meant to be
 * indistinguishable from the other adapters to the cognition engine, and is
 * typed against hand-written duck types for Puppeteer's `Page` that only a
 * real browser can catch drifting out from under.
 */
...
describe("PuppeteerAdapter against a real browser", () => {
  ...
  it("actuates: clicking, typing, scrolling, history and native dialogs", async () => {
    ...
    try {
      ...
      // Click the CTA at its perceived centre and confirm the browser
      // navigated — the exact click-then-perceive pattern the navigation
      // retry wrapper exists to protect (see `navigationRetry.ts`).
