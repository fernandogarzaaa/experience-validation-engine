...
export function renderPanelMarkdown(panel: PanelResult): string {
  ...
  push(
    `- Completion rate: **${Math.round(executive.completionRate * 100)}%** · Abandonment: **${Math.round(executive.abandonmentRate * 100)}%**`,
  );
  push();

  push("## Top Priorities");
  push();
  if (executive.topPriorities.length === 0)
    push("_No high-priority issues — the experience is solid across personas._");
  ...
  if (executive.consensusIssues.length === 0) push("_No consensus issues._");
    ...
    for (const i of executive.consensusIssues.slice(0, 15)) {
      push(
        `| ${i.title.replaceAll("|", "\\|")} | ${i.severity} | ${Math.round(i.agreement * 100)}% | ${i.personas.join(", ")} |`,
      );
    }
  }
  push();
  ...
  for (const item of critique.items.slice(0, 12)) {
    push(
      `- **[${item.severity}] ${item.title}** (${item.heuristic}) — ${item.detail} _→ ${item.recommendation}_`,
    );
  }
  push();
  ...
  if (forecast.struggles.length > 0) {
    ...
    for (const s of forecast.struggles.slice(0, 6)) {
      push(
        `- ${s.location} — struggle index ${s.struggleIndex.toFixed(2)} (${s.signals.join(", ")})`,
      );
    }
    push();
  }
  if (forecast.recommendedChanges.length > 0) {
    ...
    for (const c of forecast.recommendedChanges) {
      push(
        `- ${c.change} — heuristic lift index +${Math.round(c.estimatedLift * 100)}%. ${c.rationale}`,
      );
    }
    push();
  }
  ...
  for (const phase of plan.roadmap) {
    ...
    for (const epicId of phase.epics) {
      ...
      if (!epic) continue;
      push(
        `- **${epic.id} · ${epic.title}** (priority ${epic.priorityScore}, +${Math.round(epic.estimatedCompletionLift * 100)}% est.) — ${epic.businessImpact}`,
      );
    }
    push();
  }
