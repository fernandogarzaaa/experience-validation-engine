...
export const PROFESSIONS: Record<string, Profession> = {
  doctor: {
    name: "doctor",
    description:
      "Clinician; scans for patient/record data, intolerant of ambiguity in critical actions.",
    vocabulary: [
      "patient",
      "record",
      "chart",
    ...
    habits: [
      "Double-checks before any irreversible clinical action",
      "Scans for patient identifiers first",
    ],
  },
  teacher: {
    name: "teacher",
  ...
  lawyer: {
    name: "lawyer",
    description:
      "Precise, risk-averse; reads terms carefully, expects document and version workflows.",
    vocabulary: [
      "document",
      "contract",
      "clause",
...
export function getProfession(name: string): Profession {
  ...
  if (!p)
    throw new Error(`Unknown profession "${name}". Known: ${Object.keys(PROFESSIONS).join(", ")}`);
  return p;
}
...
export function applyProfession(base: Persona, profession: Profession): Persona {
  ...
  for (const [key, delta] of Object.entries(profession.traitDeltas) as Array<
    [keyof Persona["traits"], number]
  >) {
