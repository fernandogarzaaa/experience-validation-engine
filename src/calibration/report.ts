...
export function renderCalibrationMarkdown(report: CalibrationReport): string {
  const lines: string[] = [
    ...
    `| Timing similarity (observed durations, both sides) | ${pct(report.timingSimilarity)} |`,
    `| Step-count similarity (efficiency — NOT timing) | ${pct(report.stepSimilarity)} |`,
    `| Trajectory similarity (aligned action comparison) | ${report.trajectorySimilarity === null ? "n/a — requires per-step human action logs (future work)" : pct(report.trajectorySimilarity)} |`,
